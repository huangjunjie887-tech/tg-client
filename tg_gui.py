    ...
    def direct_login(self):
        ...
        async def login():
            ...
            try:
                ...
                # 2. 获取用户信息（可能触发 SessionPasswordNeededError）
                try:
                    me = await client.get_me()
                    # 登录成功
                    ...
                    if twofa:
                        try:
                            ...
                            error_msg = str(e2)
                            if "password" in error_msg.lower() or "PASSWORD_HASH_INVALID" in error_msg:
                                ...
                                self.direct_status.config(text=f"2FA验证失败", foreground="red")
                    else:
