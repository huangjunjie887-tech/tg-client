...
class TelegramFullGUI:
    ...
    def create_send_page(self):
        ...
        ttk.Label(target_frame, text="目标群链接列表(每行一个):").pack(anchor="w", padx=5, pady=5)
        self.group_target_text = scrolledtext.ScrolledText(target_frame, width=80, height=5)
        self.group_target_text.pack(fill="x", padx=5, pady=5)
        ttk.Button(target_frame, text="导入群链接文件", command=self.import_group_targets).pack(anchor="w", padx=5, pady=5)
    ...
    def import_group_targets(self):
        ...
        if file_path:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
            self.group_target_text.delete("1.0", tk.END)
            self.group_target_text.insert("1.0", content)
            self.group_log_insert(f"导入群组链接文件: {file_path}")
    ...
    def load_user_list_for_send(self):
        if not hasattr(self, 'save_path') or not self.save_path.get():
            return None
        save_dir = self.save_path.get().strip()
    ...
    def start_private_send(self):
        ...
        users = self.load_user_list_for_send()
        if not users:
            self.private_log_insert("请先在采集页面采集群成员并选择保存目录")
            self.show_centered_warning("提示", "请先在采集页面采集群成员并选择保存目录")
            return
        ...
        self.private_log_insert(f"目标用户: {len(users)} | 账号: {len(selected_accounts)} | 每号限: {per_account_limit}人")
        
        def run_private_send():
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.run_until_complete(self.do_private_send(selected_accounts, users, ad_text, image_path, interval, per_account_limit, thread_cnt, auto_skip))
            loop.close()
            self.private_send_running = False
    ...
    def start_group_send(self):
        ...
        target_text = self.group_target_text.get("1.0", tk.END).strip()
        if not target_text:
            self.group_log_insert("请输入目标群组链接")
            self.show_centered_warning("提示", "请输入目标群组链接")
            return
        ...
        self.group_log_insert(f"目标群组: {len(targets)}个 | 账号: {len(selected_accounts)}个 | 每号限: {per_account_limit}条")
        
        def run_group_send():
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.run_until_complete(self.do_group_send(selected_accounts, targets, ad_text, image_path, concurrent, per_account_limit, interval, thread_cnt, auto_skip))
            loop.close()
            self.group_send_running = False
